...
def Like_Contest() : # 공모전 인기공고
    ...
    like_contests = soup.select('div.items a.item') # a.item 부분 가져오기
    result_list = []

    for i, like_contest in enumerate(like_contests):
        if i == 4:  # 4번째 항목까지만 크롤링하도록 수정
            break
        title = like_contest.select_one('h3').text
        ...
        if dday == '마감' : # 마감이면 안가져오기
            continue
        link = 'https://www.campuspick.com' + like_contest['href']  # 수정된 부분
