...
def search_contest(keyword='', start_index=0) : # 공모전 검색 함수
    ...
    search_contest_list = soup.select('a.top') # 검색결과가 제목, 링크 담고있는 요소 선택
    
    result_list = [] # 크롤링 결과 담을 리스트
    
    # 시작 인덱스가 리스트 범위를 벗어나면 함수 종료
    if start_index >= len(search_contest_list) :
        driver.close()
        return result_list

    for search_contest in search_contest_list[start_index : start_index+10] : # 10개씩 가져오기
        title = search_contest.select_one('h2').text # 제목 
        company = search_contest.select_one('p.company').text # 주최기관
        dday_element = search_contest.select_one('p.info span.dday') # 디데이
        dday = dday_element.text.strip() if dday_element else 'No D-day' # 디데이
        if dday == '마감' : # 마감이면 안가져오기
            continue
        link = 'https://www.campuspick.com' + search_contest['href'] # 링크
        image_url = search_contest.select_one('figure')['data-image'] # 이미지 url 가져오기
        ...
        result_list.append(search_contest_info)
